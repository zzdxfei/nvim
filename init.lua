...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      map('n', '<leader>th', builtin.help_tags, { desc = '[S]earch [H]elp' })
      map('n', '<leader>tk', builtin.keymaps, { desc = '[S]earch [K]eymaps' })
      map('n', '<leader>tf', builtin.find_files, { desc = '[S]earch [F]iles' })
      map('n', '<leader>ts', builtin.builtin, { desc = '[S]earch [S]elect Telescope' })
      map('n', '<leader>tw', builtin.grep_string, { desc = '[S]earch current [W]ord' })
      map('n', '<leader>tg', builtin.live_grep, { desc = '[S]earch by [G]rep' })
      map('n', '<leader>td', builtin.diagnostics, { desc = '[S]earch [D]iagnostics' })
      map('n', '<leader>tr', builtin.resume, { desc = '[S]earch [R]esume' })
      map('n', '<leader>t.', builtin.oldfiles, { desc = '[S]earch Recent Files ("." for repeat)' })
      map('n', '<leader>tb', builtin.buffers, { desc = '[S] Find existing buffers' })
  ...
  },
  {
    ...
    config = function()
      ...
      vim.keymap.set({'n', 't'}, '<F4>', '<cmd>ToggleTerm<cr>', { desc = 'Toggle Terminal' })
    end
  }
}, {
  ui = {
    icons = not vim.g.have_nerd_font and {} or {
